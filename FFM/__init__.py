...
def inference(input_x, input_x_field, zeroWeights, oneDimWeights, thirdWeight):
    ...
    for i in range(input_shape):
        ...
        for j in range(i+1, input_shape):
            ...
            vector_Left =tf.convert_to_tensor([[featureIndex1,fieldIndex2,i] for i in range(vector_dimension)])
            weightLeft = tf.gather_nd(thirdWeight,vector_Left)
            weightLeftAfterCut = tf.squeeze(weightLeft)

            vector_Right = tf.convert_to_tensor([[featureIndex2,fieldIndex1,i] for i in range(vector_dimension)])
            weightRight = tf.gather_nd(thirdWeight,vector_Right)
            ...
            xi = tf.squeeze(tf.gather_nd(input_x,indice2))
            xj = tf.squeeze(tf.gather_nd(input_x,indice3))
